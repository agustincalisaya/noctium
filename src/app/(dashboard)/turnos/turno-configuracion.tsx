...
export function TurnoConfiguracion({ id, retorno }: { id?: string; retorno: string }) {
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      if (actual) {
        ...
        const cargados = { fecha: actual.fecha, hora_inicio: actual.hora_inicio, materia_id: actual.materia_id };
  ...
  const configuracionValida = Boolean(parametros && momento && campos.fecha && !fechaInvalida && campos.hora_inicio && !horaInvalida && campos.materia_id);
    ...
    try {
      ...
      if (!idGuardado || configuracionCambiada) {
        const respuesta = await fetchAutenticado(idGuardado ? `/api/turnos/${encodeURIComponent(idGuardado)}/configuracion` : "/api/turnos", {
          method: idGuardado ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(campos), cache: "no-store",
        });
        const valor = await respuesta.json().catch(() => null);
        if (!respuesta.ok) {
