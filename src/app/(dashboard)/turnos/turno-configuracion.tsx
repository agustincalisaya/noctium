...
type Campos = { fecha: string; hora_inicio: string; materia_id: string; cupo_maximo: string };
const inicial: Campos = { fecha: "", hora_inicio: "", materia_id: "", cupo_maximo: "" };
const campoError: Record<string, keyof Campos> = { FECHA_PASADA: "fecha", DIA_NO_OPERATIVO: "fecha", ANTICIPACION_EXCEDIDA: "fecha", HORA_NO_GRANULAR: "hora_inicio", FUERA_DE_HORARIO_OPERATIVO: "hora_inicio", MATERIA_NO_DISPONIBLE: "materia_id", CUPO_MENOR_A_INSCRIPTOS: "cupo_maximo" };
const CUPO_MAXIMO_PERMITIDO = 2_147_483_647;
const dias = ["DOMINGO", "LUNES", "MARTES", "MIERCOLES", "JUEVES", "VIERNES", "SABADO"];

// Mismo criterio que ConfigurarTurnoSchema (entero > 0); el tope evita
// desbordar la columna INTEGER de Postgres.
function errorDeCupo(cupo: string) {
  if (!cupo) return "";
  if (!/^\d+$/.test(cupo) || Number(cupo) <= 0) return "El cupo máximo debe ser un número entero mayor que cero";
  if (Number(cupo) > CUPO_MAXIMO_PERMITIDO) return `El cupo máximo no puede superar ${CUPO_MAXIMO_PERMITIDO}`;
  return "";
}
...
export function TurnoConfiguracion({ id, retorno }: { id?: string; retorno: string }) {
  ...
  const [resultado, setResultado] = useState<{ id: string; fecha: string; hora_inicio: string; hora_fin: string; cupo_maximo: number; profesor_desasignado?: boolean } | null>(null);
  ...
  const cargar = useCallback(async () => {
    ...
    try {
      ...
      if (id) {
        ...
        const cargados = { fecha: actual.fecha, hora_inicio: actual.hora_inicio, materia_id: actual.materia_id, cupo_maximo: String(actual.cupo_maximo) };
        setOriginales(cargados);
        setCampos({ ...cargados, hora_inicio: horaVigente ? actual.hora_inicio : "" });
  ...
  const sinGuardar = !resultado && (Object.keys(campos) as (keyof Campos)[]).some((campo) => campos[campo] !== originales[campo]);
  ...
  const cupoInvalido = errorDeCupo(campos.cupo_maximo);
  const puedeGuardar = Boolean(parametros && momento && campos.fecha && !fechaInvalida && campos.hora_inicio && !horaInvalida && campos.materia_id && campos.cupo_maximo && !cupoInvalido && !guardando);
  ...
  const guardar = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const respuesta = await fetchAutenticado(id ? `/api/turnos/${encodeURIComponent(id)}/configuracion` : "/api/turnos", {
        method: id ? "PATCH" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...campos, cupo_maximo: Number(campos.cupo_maximo) }), cache: "no-store",
      });
  ...
  return <main className="mx-auto w-full min-w-0 max-w-2xl space-y-5 p-6">
    ...
    {cargando ? <p role="status">Cargando configuración</p> : errorCarga ? <div role="alert" className="space-y-3 rounded-md border border-border bg-card p-4"><p>{errorCarga}</p><Button variant="outline" onClick={() => void cargar()}>Reintentar</Button></div> : resultado ? <div role="status" className="space-y-3 rounded-md bg-success p-5 text-success-foreground"><p className="font-semibold">{id ? "Configuración actualizada" : "Turno configurado"}</p><p>{resultado.fecha} · {resultado.hora_inicio}–{resultado.hora_fin} · Cupo máximo: {resultado.cupo_maximo} · Pendiente</p>{resultado.profesor_desasignado && <p>El profesor dejó de corresponder a la materia y fue desasignado. Debe reasignarse.</p>}<div className="flex flex-wrap gap-4"><Link className="underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" href={retorno} prefetch={false}>Volver al listado</Link><Link className="underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" href={`/turnos/${encodeURIComponent(resultado.id)}?volver=${encodeURIComponent(retorno)}`} prefetch={false}>Ver detalle</Link>{!id && <Link className="underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" href={`/turnos/${encodeURIComponent(resultado.id)}/participantes?volver=${encodeURIComponent(retorno)}`} prefetch={false}>Continuar con alumno y profesor</Link>}</div></div> : turno && turno.estado !== "PENDIENTE" ? <p role="alert">Un turno disponible o completo no admite cambios de configuración.</p> : materias.length === 0 ? <p role="status">No hay materias activas para configurar turnos</p> : <form noValidate onSubmit={(event) => void guardar(event)} className="space-y-5 rounded-md border border-border bg-card p-5 text-card-foreground">
      ...
      <div className="space-y-1"><label htmlFor="cupo" className="text-sm font-medium">Cupo máximo *</label><Input id="cupo" type="number" inputMode="numeric" required min={1} max={CUPO_MAXIMO_PERMITIDO} step={1} value={campos.cupo_maximo} onChange={(e) => cambiar("cupo_maximo", e.target.value)} aria-invalid={Boolean(errores.cupo_maximo || cupoInvalido)} aria-describedby={errores.cupo_maximo || cupoInvalido ? "cupo-error" : "cupo-ayuda"} /><p id="cupo-ayuda" className="text-sm text-muted-foreground">Cantidad máxima de alumnos que pueden inscribirse al turno.</p>{(errores.cupo_maximo || cupoInvalido) && <p id="cupo-error" role="alert" className="text-sm text-destructive">{errores.cupo_maximo || cupoInvalido}</p>}</div>
