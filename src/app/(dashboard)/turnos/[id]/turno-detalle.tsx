...
export function TurnoDetalleVista({ id, retorno, puedeConfigurar, puedeGestionarAlumnos, puedeAsignarAula }: { id: string; retorno: string; puedeConfigurar: boolean; puedeGestionarAlumnos: boolean; puedeAsignarAula: boolean }) {
  ...
  return <main className="mx-auto w-full min-w-0 max-w-3xl space-y-5 p-6"><Link className="rounded-sm text-sm text-primary underline underline-offset-4 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" href={retorno} prefetch={false}>Volver al listado</Link><h1 className="text-2xl font-semibold">Detalle del turno</h1>{turno?.estado === "PENDIENTE" && (puedeConfigurar || puedeAsignarAula) && <div className="flex flex-wrap gap-3">{puedeConfigurar && <><Link className="inline-flex rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" href={`/turnos/${encodeURIComponent(id)}/configuracion?volver=${encodeURIComponent(retorno)}`} prefetch={false}>Modificar configuración</Link><Link className="inline-flex rounded-md border border-border px-4 py-2 text-sm font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" href={`/turnos/${encodeURIComponent(id)}/participantes?volver=${encodeURIComponent(retorno)}`} prefetch={false}>Asignar o cambiar participantes</Link></>}{puedeAsignarAula && <Link className="inline-flex rounded-md border border-border px-4 py-2 text-sm font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring" href={`/turnos/${encodeURIComponent(id)}/aula?volver=${encodeURIComponent(retorno)}`} prefetch={false}>Asignar o cambiar aula</Link>}</div>}
...
function InscripcionesTurno({ turno, onCambio }: { turno: TurnoDetalle; onCambio: () => Promise<void> }) {
  ...
  const cupoAlcanzado = turno.alumnos.length >= turno.cupo_maximo;
  ...
  return <section aria-labelledby="inscripciones-titulo" className="space-y-4 rounded-md border border-border bg-card p-5 text-card-foreground">
    <h2 id="inscripciones-titulo" className="text-lg font-semibold">Alumnos inscriptos <span className="font-normal text-muted-foreground">({turno.alumnos.length}/{turno.cupo_maximo})</span></h2>
